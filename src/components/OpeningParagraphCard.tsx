      ...
      <div className="flex flex-col justify-center h-full">
        {isLoading ? (
          ...
          <div className="w-full flex flex-col gap-6 md:gap-8">
            {title && (
              <h2 className="font-serif text-3xl md:text-4xl lg:text-5xl font-normal text-brand-ink tracking-tight">
                {title}
              </h2>
            )}
            
            <div className="font-serif text-xl md:text-2xl lg:text-[1.65rem] text-brand-ink/90 leading-[1.8] space-y-6 md:space-y-8">
              {openingParagraph && openingParagraph.split("\n\n").map((para, idx) => (
                <p key={`opening-${idx}`} className={idx === 0 ? "first-letter:text-[3.8rem] first-letter:leading-[0.8] first-letter:mr-3.5 first-letter:float-left first-letter:font-serif first-letter:text-brand-ink" : ""}>
                  {para}
                </p>
              ))}
            ...
            {isContinuing && (
              <div className="font-sans text-xs text-brand-ink/45 flex items-center gap-2 animate-pulse mt-4 font-normal tracking-wide">
